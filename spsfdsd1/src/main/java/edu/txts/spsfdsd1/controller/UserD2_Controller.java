...
public class UserD2_Controller {
	@Autowired
	private UserD2_Service userD2_Service;

	@PostMapping
	public ApiResponse<UserD2> createUserD2(@RequestBody @Valid UserD2_Create userD2) {
		ApiResponse<UserD2> apiResponse = new ApiResponse<>();
		apiResponse.setResult(userD2_Service.createUserD2(userD2));
		return apiResponse;
	}
	...
	@GetMapping({"/{id}"})
	public UserD2 getUserD2(@PathVariable String id) {
		return userD2_Service.getUserD2(id);
	}

	@PutMapping({"/{id}"})
	public UserD2 updateUserD2(@PathVariable String id, @RequestBody  UserD2Update userD2Update) {
